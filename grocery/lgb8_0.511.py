...
#        "trans_7_2017":  get_timespan(df_transactions_train, t2017, 7, 7).mean(axis=1).values,   
#        "trans_14_2017":  get_timespan(df_transactions_train, t2017, 14, 14).mean(axis=1).values,   
#        "trans_30_2017":  get_timespan(df_transactions_train, t2017, 30, 30).mean(axis=1).values,   
...
#       "weighted_mean_week": get_timespan(df_2017, t2017, 7, 7).mean(axis=1).values*0.4 + get_timespan(df_2017, t2017, 14, 7).mean(axis=1).values*0.3 + get_timespan(df_2017, t2017, 21, 7).mean(axis=1).values*0.2 + get_timespan(df_2017, t2017, 28, 7).mean(axis=1).values*0.1,         
        ...
        "var_30_2017": get_timespan(df_2017, t2017, 30, 30).var(axis=1).values,
...
#        X['trans_median_8_dow{}_2017'.format(i)] =get_timespan(df_transactions_train, t2017, 56-i, 8, freq='7D').median(axis=1).values   
        ...
        for m in [3,7,14,30,60,140]:
            ...
            X["mean_{}_2017_{}2".format(m,i)]= get_timespan(df_2017, date-timedelta(7),m, m).\
                mean(axis=1).values
    X.drop(['bf_unpromo_15','mean_140_2017_01'], axis=1, inplace=True)
    if is_train:
        y = df_2017[
...
submission[['id','unit_sales']].to_csv('lgb0107-2.csv', float_format='%.4f', index=None)    
