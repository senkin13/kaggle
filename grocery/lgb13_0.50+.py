...
#create item id feature
df_item = df2017.set_index(
    ["store_nbr", "item_nbr", "date"])[["item"]].unstack(
        level=-1).fillna(-1)
df_item.columns = df_item.columns.get_level_values(1)
df_item['item'] = df_item.max(axis=1)
df_item = df_item['item']
df_item_train2 = pd.get_dummies(df_item, columns=['item'], prefix='item', drop_first=True)
del df_item
...
del df_items_train,df_family_train,df_class_train,df_perishable_train
...
del df_stores_train,df_city_train,df_state_train,df_type_train,df_cluster_train
...
#        "range_30_2017": get_timespan(df_2017, t2017, 30, 30).max(axis=1).values - np.nan_to_num(get_timespan(df_2017, t2017, 30, 30)[get_timespan(df_2017, t2017, 30, 30) > 0].min(axis=1).values),            
 ...
 #       "per25_7_2017": get_timespan(df_2017, t2017, 7, 7).quantile(.25, axis=1).values, 
    ...
    for c in df_item_train2.columns:
        X[c] = df_item_train2[c].values
...
del df_item_train2,df_store_train2,df_family_train2,df_class_train2,df_perishable_train2,df_city_train2,df_state_train2,df_type_train2,df_cluster_train2
gc.collect()
...
submission[['id','unit_sales']].to_csv('lgb0110-ohe-all.csv', float_format='%.6f', index=None)    
