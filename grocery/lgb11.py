...
print("Preparing features for 1 week...")
...
#        "range_30_2017": get_timespan(df_2017, t2017, 30, 30).max(axis=1).values - np.nan_to_num(get_timespan(df_2017, t2017, 30, 30)[get_timespan(df_2017, t2017, 30, 30) > 0].min(axis=1).values),            
 ...
 #       "per25_7_2017": get_timespan(df_2017, t2017, 7, 7).quantile(.25, axis=1).values, 
        ...
        "unpromo_7aftsum_2017":(1-get_timespan(promo_2017, t2017+timedelta(7), 7, 7)).iloc[:,1:].sum(axis=1).values, 
        })
    ...
    for i in range(7):
        ...
        X["bf_unpromo_{}".format(i)] = (1-get_timespan(
                promo_2017, t2017+timedelta(7), 7-i, 7-i)).iloc[:,1:].sum(
                        axis=1).values / (6-i) * X['promo_{}'.format(i)]
...
#        X['median_20_dow{}_2017'.format(i)] = get_timespan(df_2017, t2017, 140-i, 20, freq='7D').median(axis=1).values         
    ...
    if is_train:
        y = df_2017[
            pd.date_range(t2017, periods=7)
        ].values
        return X, y
    return X
    
    
print("Preparing dataset for 1 week...")
...
X_val, y_val = prepare_dataset(date(2017, 8, 2))
X_test = prepare_dataset(date(2017, 8, 16), is_train=False)

print("Training and predicting 1 week lgb models...")
params = {
    'num_leaves': 33,
...
print("Preparing features for 2 week...")
...
#        "range_30_2017": get_timespan(df_2017, t2017, 30, 30).max(axis=1).values - np.nan_to_num(get_timespan(df_2017, t2017, 30, 30)[get_timespan(df_2017, t2017, 30, 30) > 0].min(axis=1).values),            
 ...
 #       "per25_7_2017": get_timespan(df_2017, t2017, 7, 7).quantile(.25, axis=1).values, 
        ...
        "promo_56_2017": get_timespan(promo_2017, t2017, 56, 56).sum(axis=1).values,
        "promo_112_2017": get_timespan(promo_2017, t2017, 112, 112).sum(axis=1).values,
        "unpromo_14aftsum_2017":(1-get_timespan(promo_2017, t2017+timedelta(14), 14, 14)).iloc[:,1:].sum(axis=1).values, 
        })
    ...
    for i in range(14):
        ...
        X["bf_unpromo_{}".format(i)] = (1-get_timespan(
                promo_2017, t2017+timedelta(14), 14-i, 14-i)).iloc[:,1:].sum(
                        axis=1).values / (13-i) * X['promo_{}'.format(i)]
...
print("Preparing dataset for 2 week...")
...
print("Training and predicting 2 week lgb models...")
...
for i in range(7):
    ...
    print("=" * 50)
    dtrain = lgb.Dataset(
        X_train, label=y_train[:, i+7],
        categorical_feature=cate_vars,
        weight=pd.concat([items["perishable"]] * 6) * 0.25 + 1
    )
    dval = lgb.Dataset(
        X_val, label=y_val[:, i+7], reference=dtrain,
        weight=items["perishable"] * 0.25 + 1,
        categorical_feature=cate_vars)
    bst = lgb.train(
        params, dtrain, num_boost_round=MAX_ROUNDS,
...
print("Preparing features for 3 week...")
...
print("Preparing dataset for 3 week...")
...
print("Training and predicting 3 week lgb models...")
...
for i in range(2):
    ...
    print("=" * 50)
    dtrain = lgb.Dataset(
        X_train, label=y_train[:, i+14],
        categorical_feature=cate_vars,
        weight=pd.concat([items["perishable"]] * 6) * 0.25 + 1
    )
    dval = lgb.Dataset(
        X_val, label=y_val[:, i+14], reference=dtrain,
        weight=items["perishable"] * 0.25 + 1,
        categorical_feature=cate_vars)
    bst = lgb.train(
        params, dtrain, num_boost_round=MAX_ROUNDS,
...
submission[['id','unit_sales']].to_csv('lgb-3models.csv', float_format='%.4f', index=None)      
